...
import { ItemDefinitionService } from './services/itemDefinitionService'
import type { DictionaryEntry, Dictionary, AnalysisProgress as ProgressType } from './types'
...
function App() {
  ...
  const handleImportJSON = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        const data = JSON.parse(event.target?.result as string) as Dictionary
  ...
  const handleAnalyze = useCallback(async () => {
    const itemDefinitionService = await ItemDefinitionService.getInstanceAsync()
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        // アイテム情報の抽出
        const result = await itemDefinitionService.extractAsync(src)
      ...
      itemDefinitionService?.dispose()
    }
  }, [images, entries.length])

  const handleUpdateEntry = (index: number, field: keyof DictionaryEntry, value: any) => {
    setEntries((prev) => {
      const updated = [...prev]
